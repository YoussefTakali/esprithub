...
public class GitHubWebhookService {
    ...
    public void updateWebhookDelivery(String webhookId, boolean success, String error) {
        Optional<WebhookSubscription> subscription = webhookSubscriptionRepository.findByWebhookId(webhookId);
        if (subscription.isPresent()) {
            WebhookSubscription sub = subscription.get();
            sub.setLastDelivery(LocalDateTime.now());
            
            if (success) {
                sub.setFailureCount(0);
                sub.setLastError(null);
                if (sub.getStatus() == WebhookSubscription.WebhookStatus.FAILED) {
                    sub.setStatus(WebhookSubscription.WebhookStatus.ACTIVE);
                }
            } else {
                sub.setFailureCount(sub.getFailureCount() + 1);
                sub.setLastError(error);
                
                if (sub.getFailureCount() >= webhookProperties.getRetry().getMaxFailures()) {
                    sub.setStatus(WebhookSubscription.WebhookStatus.FAILED);
                }
            }
